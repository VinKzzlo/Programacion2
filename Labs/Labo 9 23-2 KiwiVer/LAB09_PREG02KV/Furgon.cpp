...
void Furgon::imprime(ofstream& arch) {
    ...
    if(not pdeposito.empty()){
        for(list<class NPedido>::iterator it = pdeposito.begin();
                it != pdeposito.end(); it++){
            (*it).imprimePedido(arch);
        }
    }
}

void Furgon::cargadeposito(NPedido& nped) {
    if(dentroMaxCarga(nped.GetPeso())){
        pdeposito.push_front(nped);
        SetActcarga(GetActcarga()+nped.GetPeso());
    }
}

bool Furgon::dentroMaxCarga(double peso) {
